...
import { type UnitStats, type StageConfig } from '../types/game';
...
const Game: React.FC = () => {
  ...
  const startGame = (stage: StageConfig) => {
    ...
    s.gameState = 'playing';
    audio.startBGM(); audio.playSystemSE(440);
  };
  ...
  useEffect(() => {
    ...
    const loop = (timestamp: number) => {
      ...
      drawGame(ctx, s, currentStage, timestamp);
      setUi({
        money: Math.floor(s.money), walletLevel: s.walletLevel, baseHp: s.baseHp, enemyBaseHp: s.enemyBaseHp, cannonCharge: Math.floor(s.cannonCharge), gameState: s.gameState,
        cooldownPercents: {
    ...
    const update = (dt: number, timestamp: number) => {
      ...
      if (s.enemySpawnTimer > currentStage.enemySpawnRate) { spawnUnit('ENEMY', false); s.enemySpawnTimer = 0; }
    ...
    const spawnUnit = (key: string, isAlly: boolean) => {
      ...
      if (!isAlly) stats.hp *= currentStage.enemyHpMultiplier;
      s.units.push({ id: s.nextUnitId++, x: isAlly ? 110 : 800 - 110, y: 400 - 70, type: isAlly ? 'ally' : 'enemy', unitType: key, stats, currentHp: stats.hp });
    };
  ...
  const handleSpawn = (type: string) => {
    ...
    if (s.money >= UNIT_TYPES[type].cost && s.cooldowns[type] <= 0) {
      ...
      s.units.push({ id: s.nextUnitId++, x: 110, y: 400 - 70, type: 'ally', unitType: type, stats: UNIT_TYPES[type], currentHp: UNIT_TYPES[type].hp });
  ...
  return (
    <div style={{ textAlign: 'center', backgroundColor: '#ecf0f1', minHeight: '100vh', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ position: 'relative', display: 'inline-block', marginBottom: '20px' }}>
        <canvas ref={canvasRef} width={800} height={400} style={{ border: '4px solid #34495e', borderRadius: '15px', backgroundColor: '#fff', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }} />
        {ui.gameState === 'start' && (
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', color: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', borderRadius: '12px' }}>
            <h2 style={{ fontSize: '32px', marginBottom: '30px' }}>ステージ選択</h2>
            <div style={{ display: 'flex', gap: '20px' }}>
              {Object.values(STAGES).map(stage => (
                <button key={stage.id} onClick={() => startGame(stage)} style={{ padding: '20px 30px', fontSize: '18px', cursor: 'pointer', borderRadius: '10px', background: stage.id === 1 ? '#2ecc71' : '#e67e22', color: '#fff', border: 'none', fontWeight: 'bold' }}>
        ...
        {(ui.gameState === 'victory' || ui.gameState === 'defeat') && (
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.7)', color: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', borderRadius: '12px' }}>
            ...
            <button onClick={() => window.location.reload()} style={{ padding: '15px 40px', fontSize: '24px', cursor: 'pointer', borderRadius: '8px', background: '#3498db', color: '#fff', border: 'none' }}>BACK TO MENU</button>
          </div>
        )}
      </div>
